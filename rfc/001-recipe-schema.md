...
Recipe `app_min_*` fields declare **app-only** resource requirements for RAM and disk. CPU cores are also declared per recipe.
...
Example: n8n (2048 MB, 2 cores) + Portainer (512 MB, 1 core) on Ubuntu 26.04 (512 MB, 1 core baseline) = **3072 MB** total RAM, **4 CPU cores** total.
...
>>>>>>> 2094ca0 (Change disk size units from GB to MB for consistency)
