...
# (e.g. "afpre-password-pusher"), so a project-label filter derived from this script's
# own directory ("password-pusher") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)password-pusher-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all password-pusher containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: password-pusher did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)password-pusher-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)password-pusher-" 2>/dev/null || true)
    for id in $ids; do
...
URL="${1:-http://${SERVERIP:-127.0.0.1}:5100}"
echo "Checking Password Pusher at ${URL}/up ..."
curl -fsS --max-time 10 "${URL}/up" > /dev/null
echo "Password Pusher is healthy."
