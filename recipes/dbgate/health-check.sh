...
# (e.g. "afpre-dbgate"), so a project-label filter derived from this script's
# own directory ("dbgate") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)dbgate-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all dbgate containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: dbgate did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)dbgate-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)dbgate-" 2>/dev/null || true)
    for id in $ids; do
...
# DbGate has no built-in authentication (gated by Traefik basic auth at the
# platform level), so the server returns 200 on a direct port probe.
URL="${1:-http://${SERVERIP:-127.0.0.1}:3000}"
echo "Checking DbGate at ${URL} ..."
curl -fsS --max-time 10 "${URL}" > /dev/null
echo "DbGate is healthy."
