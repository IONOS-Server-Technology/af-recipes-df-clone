...
# (e.g. "afpre-karaoke-eternal"), so a project-label filter derived from this script's
# own directory ("karaoke-eternal") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)karaoke-eternal-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all karaoke-eternal containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: karaoke-eternal did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)karaoke-eternal-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)karaoke-eternal-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Karaoke Eternal at ${URL}/ ..."
curl -fsS --max-time 10 "${URL}/" > /dev/null
echo "Karaoke Eternal is healthy."
