#!/usr/bin/env bash
# install.sh — Install Chevereto via docker-compose
set -euo pipefail
...
# break a database already initialised with the old value.
af_gen_secret() {
    local key="$1" value
...
mkdir -p /opt/chevereto/content
mkdir -p /opt/chevereto/mariadb
