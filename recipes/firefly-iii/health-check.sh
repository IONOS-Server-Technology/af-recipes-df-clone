...
# (e.g. "afpre-firefly-iii"), so a project-label filter derived from this script's
# own directory ("firefly-iii") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)firefly-iii-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all firefly-iii containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: firefly-iii did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)firefly-iii-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)firefly-iii-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Firefly III at ${URL} ..."
