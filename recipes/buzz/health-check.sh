...
# (e.g. "afpre-buzz"), so a project-label filter derived from this script's
# own directory ("buzz") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)buzz-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all buzz containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: buzz did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)buzz-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)buzz-" 2>/dev/null || true)
    for id in $ids; do
...
# Port 3000 is the Buzz relay's WebSocket endpoint; nc checks TCP connectivity
# without triggering a WebSocket handshake that would require a Nostr client.
SERVERIP="${1:-${SERVERIP:-127.0.0.1}}"
echo "Checking Buzz relay at ${SERVERIP}:3000 ..."
nc -z "${SERVERIP}" 3000
echo "Buzz relay is reachable."
