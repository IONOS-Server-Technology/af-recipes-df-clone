...
# (e.g. "afpre-silex"), so a project-label filter derived from this script's
# own directory ("silex") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)silex-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all silex containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: silex did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)silex-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)silex-" 2>/dev/null || true)
    for id in $ids; do
...
URL="${1:-http://${SERVERIP:-127.0.0.1}:6805}"
echo "Checking Silex at ${URL} ..."
curl -fsS --max-time 10 "${URL}" > /dev/null
echo "Silex is healthy."
