...
# (e.g. "afpre-metabase"), so a project-label filter derived from this script's
# own directory ("metabase") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)metabase-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all metabase containers are healthy after ${waited}s"
            return 0
        fi
    ...
    docker ps -a --filter "name=(^|-)metabase-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)metabase-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Metabase at ${URL}/api/health ..."
