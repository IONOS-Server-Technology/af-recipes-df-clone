...
# (e.g. "afpre-corteza"), so a project-label filter derived from this script's
# own directory ("corteza") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)corteza-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all corteza containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: corteza did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)corteza-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)corteza-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Corteza at ${URL}/healthz ..."
curl -fsS --max-time 10 "${URL}/healthz" > /dev/null
echo "Corteza is healthy."
