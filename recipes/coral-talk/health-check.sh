...
# (e.g. "afpre-coral-talk"), so a project-label filter derived from this script's
# own directory ("coral-talk") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)coral-talk-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all coral-talk containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: coral-talk did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)coral-talk-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)coral-talk-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Coral Talk at ${URL}/api/health ..."
curl -fsS --max-time 10 "${URL}/api/health" > /dev/null
echo "Coral Talk is healthy."
