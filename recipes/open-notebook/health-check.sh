...
# (e.g. "afpre-open-notebook"), so a project-label filter derived from this script's
# own directory ("open-notebook") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)open-notebook-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all open-notebook containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: open-notebook did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)open-notebook-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)open-notebook-" 2>/dev/null || true)
    for id in $ids; do
...
URL="${1:-http://${SERVERIP:-127.0.0.1}:8502}"
echo "Checking Open Notebook at ${URL}/_stcore/health ..."
curl -fsS --max-time 10 "${URL}/_stcore/health" > /dev/null
echo "Open Notebook is healthy."
