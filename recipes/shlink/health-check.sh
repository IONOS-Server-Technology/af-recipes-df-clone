...
# (e.g. "afpre-shlink"), so a project-label filter derived from this script's
# own directory ("shlink") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)shlink-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all shlink containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: shlink did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)shlink-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)shlink-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Shlink at ${URL}/rest/v3/health ..."
curl -fsS --max-time 10 "${URL}/rest/v3/health" > /dev/null
echo "Shlink is healthy."
