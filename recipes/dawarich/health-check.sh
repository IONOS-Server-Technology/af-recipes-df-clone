...
# (e.g. "afpre-dawarich"), so a project-label filter derived from this script's
# own directory ("dawarich") never matches and silently waits out the full
...
wait_for_healthy() {
    local max_wait=600 waited=0 ids id state status code name pending
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)dawarich-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all dawarich containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: dawarich did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)dawarich-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)dawarich-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Dawarich at ${URL}/api/v1/health ..."
curl -fsS --max-time 10 "${URL}/api/v1/health" > /dev/null
echo "Dawarich is healthy."
