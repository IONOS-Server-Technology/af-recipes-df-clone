...
# (e.g. "afpre-kafbat-kafka-ui"), so a project-label filter derived from this script's
# own directory ("kafbat-kafka-ui") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)kafbat-kafka-ui-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all kafbat-kafka-ui containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: kafbat-kafka-ui did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)kafbat-kafka-ui-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)kafbat-kafka-ui-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Kafbat Kafka UI at ${URL}/actuator/health ..."
curl -fsS --max-time 10 "${URL}/actuator/health" > /dev/null
echo "Kafbat Kafka UI is healthy."
