#!/usr/bin/env bash
# install.sh — Install Blinko via docker-compose
set -euo pipefail
...
# break a database or NextAuth sessions already initialised with the old value.
af_gen_secret() {
    local key="$1" value
...
mkdir -p /opt/blinko/postgres
mkdir -p /opt/blinko/data
