...
# (e.g. "afpre-umami-analytics"), so a project-label filter derived from this script's
# own directory ("umami-analytics") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)umami-analytics-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all umami-analytics containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: umami-analytics did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)umami-analytics-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)umami-analytics-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Umami Analytics at ${URL}/api/heartbeat ..."
curl -fsS --max-time 10 "${URL}/api/heartbeat" > /dev/null
echo "Umami Analytics is healthy."
