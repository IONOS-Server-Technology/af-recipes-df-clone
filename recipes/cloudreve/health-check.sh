...
# (e.g. "afpre-cloudreve"), so a project-label filter derived from this script's
# own directory ("cloudreve") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)cloudreve-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all cloudreve containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: cloudreve did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)cloudreve-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)cloudreve-" 2>/dev/null || true)
    for id in $ids; do
...
URL="${1:-http://${SERVERIP:-127.0.0.1}:5212}"
echo "Checking Cloudreve at ${URL}/api/v3/ping ..."
curl -fsS --max-time 10 "${URL}/api/v3/ping" > /dev/null
echo "Cloudreve is healthy."
