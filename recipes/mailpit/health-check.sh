...
# (e.g. "afpre-mailpit"), so a project-label filter derived from this script's
# own directory ("mailpit") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)mailpit-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all mailpit containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: mailpit did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)mailpit-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)mailpit-" 2>/dev/null || true)
    for id in $ids; do
...
# curl without -f exits 0 for any HTTP response (including 401 when MP_UI_AUTH is set),
# and non-zero only on connection failure — so a running but auth-protected Mailpit
# instance still reports healthy here.
URL="${1:-http://${SERVERIP:-127.0.0.1}:8025}"
echo "Checking Mailpit at ${URL}/api/v1/info ..."
curl -sS --max-time 10 --output /dev/null "${URL}/api/v1/info"
