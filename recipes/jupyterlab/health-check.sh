...
# (e.g. "afpre-jupyterlab"), so a project-label filter derived from this script's
# own directory ("jupyterlab") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)jupyterlab-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all jupyterlab containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: jupyterlab did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)jupyterlab-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)jupyterlab-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking JupyterLab at ${URL}/ ..."
