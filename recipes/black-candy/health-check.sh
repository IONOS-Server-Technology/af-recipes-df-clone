...
# (e.g. "afpre-black-candy"), so a project-label filter derived from this script's
# own directory ("black-candy") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)black-candy-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all black-candy containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: black-candy did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)black-candy-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)black-candy-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Black Candy at ${URL}/ ..."
curl -fsS --max-time 10 "${URL}/" > /dev/null
echo "Black Candy is healthy."
