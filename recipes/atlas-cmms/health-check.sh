...
# (e.g. "afpre-atlas-cmms"), so a project-label filter derived from this script's
# own directory ("atlas-cmms") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)atlas-cmms-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all atlas-cmms containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: atlas-cmms did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)atlas-cmms-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)atlas-cmms-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Atlas CMMS at ${URL} ..."
wget -qO- --timeout=10 "${URL}/" >/dev/null
echo "Atlas CMMS is healthy."
