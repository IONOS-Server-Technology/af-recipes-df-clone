...
# (e.g. "afpre-flagsmith"), so a project-label filter derived from this script's
# own directory ("flagsmith") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)flagsmith-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all flagsmith containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: flagsmith did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)flagsmith-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)flagsmith-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Flagsmith at ${URL}/api/v1/ ..."
curl -fsS --max-time 10 "${URL}/api/v1/" > /dev/null
echo "Flagsmith is healthy."
