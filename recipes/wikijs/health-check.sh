...
# (e.g. "afpre-wikijs"), so a project-label filter derived from this script's
# own directory ("wikijs") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)wikijs-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all wikijs containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: wikijs did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)wikijs-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)wikijs-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Wiki.js at ${URL} ..."
curl -fsS --max-time 10 "${URL}" > /dev/null
echo "Wiki.js is healthy."
