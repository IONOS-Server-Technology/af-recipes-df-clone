...
# (e.g. "afpre-apache-answer"), so a project-label filter derived from this script's
# own directory ("apache-answer") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)apache-answer-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all apache-answer containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: apache-answer did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)apache-answer-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)apache-answer-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Apache Answer at ${URL}/ ..."
curl -fsS --max-time 10 "${URL}/" > /dev/null
echo "Apache Answer is healthy."
