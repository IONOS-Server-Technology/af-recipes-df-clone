#!/usr/bin/env bash
# install.sh — Install Saltcorn via docker-compose
set -euo pipefail
...
#
# Idempotent on purpose: an existing non-empty value is left alone.
af_gen_secret() {
    local key="$1" value
...
af_gen_secret PGPASSWORD
af_gen_secret SALTCORN_SESSION_SECRET
...
mkdir -p /opt/saltcorn/data
mkdir -p /opt/saltcorn/postgres
