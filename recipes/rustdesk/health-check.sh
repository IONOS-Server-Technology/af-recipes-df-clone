...
# (e.g. "afpre-rustdesk"), so a project-label filter derived from this script's
# own directory ("rustdesk") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)rustdesk-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all rustdesk containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: rustdesk did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)rustdesk-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)rustdesk-" 2>/dev/null || true)
    for id in $ids; do
...
HOST="${1:-${SERVERIP:-127.0.0.1}}"
echo "Checking RustDesk hbbr relay at ${HOST}:21117 ..."
nc -z -w 5 "${HOST}" 21117
echo "RustDesk is healthy."
