...
# (e.g. "afpre-autobrr"), so a project-label filter derived from this script's
# own directory ("autobrr") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)autobrr-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all autobrr containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: autobrr did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)autobrr-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)autobrr-" 2>/dev/null || true)
    for id in $ids; do
...
URL="${1:-http://${SERVERIP:-127.0.0.1}:7474}"
echo "Checking autobrr at ${URL} ..."
curl -fsS --max-time 10 "${URL}/" > /dev/null
echo "autobrr is healthy."
