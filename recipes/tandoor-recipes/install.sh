#!/usr/bin/env bash
# install.sh — Install Tandoor Recipes via docker-compose
set -euo pipefail
...
#
# Idempotent: an existing non-empty value is left alone. Rotating these secrets on
# a re-run would break the database connection and invalidate Django sessions.
af_gen_secret() {
    local key="$1" value
...
mkdir -p /opt/tandoor-recipes/mediafiles
mkdir -p /opt/tandoor-recipes/postgres
