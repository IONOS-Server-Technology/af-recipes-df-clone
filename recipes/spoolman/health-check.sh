...
# (e.g. "afpre-spoolman"), so a project-label filter derived from this script's
# own directory ("spoolman") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)spoolman-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all spoolman containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: spoolman did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)spoolman-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)spoolman-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Spoolman at ${URL}/api/v1/health ..."
curl -fsS --max-time 10 "${URL}/api/v1/health" > /dev/null
echo "Spoolman is healthy."
