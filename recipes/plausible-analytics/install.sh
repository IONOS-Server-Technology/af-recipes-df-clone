#!/usr/bin/env bash
# install.sh — Install Plausible Analytics via docker-compose
set -euo pipefail
...
# SECRET_KEY_BASE requires ≥64 bytes (128 hex chars) for Plausible session signing.
...
mkdir -p /opt/plausible-analytics/postgres
mkdir -p /opt/plausible-analytics/clickhouse
