...
# (e.g. "afpre-plausible-analytics"), so a project-label filter derived from this script's
# own directory ("plausible-analytics") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)plausible-analytics-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all plausible-analytics containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: plausible-analytics did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)plausible-analytics-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)plausible-analytics-" 2>/dev/null || true)
    for id in $ids; do
...
URL="${1:-http://${SERVERIP:-127.0.0.1}:8000}"
echo "Checking Plausible Analytics at ${URL}/api/health ..."
curl -fsS --max-time 10 "${URL}/api/health" > /dev/null
echo "Plausible Analytics is healthy."
