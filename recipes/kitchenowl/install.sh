#!/usr/bin/env bash
# install.sh — Install KitchenOwl via docker-compose
set -euo pipefail

# Generate this server's own JWT_SECRET_KEY before anything reads it.
# Ships empty — a fixed value here would be identical on every server.
#
