...
# (e.g. "afpre-jenkins"), so a project-label filter derived from this script's
# own directory ("jenkins") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)jenkins-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all jenkins containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: jenkins did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)jenkins-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)jenkins-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking Jenkins at ${URL}/login ..."
curl -fsS --max-time 10 "${URL}/login" > /dev/null
