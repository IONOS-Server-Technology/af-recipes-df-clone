...
# (e.g. "afpre-grist"), so a project-label filter derived from this script's
# own directory ("grist") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)grist-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all grist containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: grist did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)grist-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)grist-" 2>/dev/null || true)
    for id in $ids; do
...
URL="${1:-http://${SERVERIP:-127.0.0.1}:8484}"
echo "Checking Grist at ${URL} ..."
curl -fsS --max-time 10 "${URL}" > /dev/null
echo "Grist is healthy."
