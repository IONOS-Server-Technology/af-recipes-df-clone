...
# (e.g. "afpre-openproject"), so a project-label filter derived from this script's
# own directory ("openproject") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)openproject-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all openproject containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: openproject did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)openproject-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)openproject-" 2>/dev/null || true)
    for id in $ids; do
...
echo "Checking OpenProject at ${URL}/health_check ..."
curl -fsS --max-time 10 "${URL}/health_check" > /dev/null
