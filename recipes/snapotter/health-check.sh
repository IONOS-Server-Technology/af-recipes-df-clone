...
# (e.g. "afpre-snapotter"), so a project-label filter derived from this script's
# own directory ("snapotter") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)snapotter-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all snapotter containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: snapotter did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)snapotter-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)snapotter-" 2>/dev/null || true)
    for id in $ids; do
...
URL="${1:-http://${SERVERIP:-127.0.0.1}:1349}"
echo "Checking SnapOtter at ${URL}/api/v1/health ..."
curl -fsS --max-time 10 "${URL}/api/v1/health" > /dev/null
echo "SnapOtter is healthy."
