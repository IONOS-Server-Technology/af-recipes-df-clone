...
# (e.g. "afpre-verdaccio"), so a project-label filter derived from this script's
# own directory ("verdaccio") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)verdaccio-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all verdaccio containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: verdaccio did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)verdaccio-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)verdaccio-" 2>/dev/null || true)
    for id in $ids; do
...
URL="${1:-http://${SERVERIP:-127.0.0.1}:4873}"
echo "Checking Verdaccio at ${URL}/-/ping ..."
curl -fsS --max-time 10 "${URL}/-/ping" > /dev/null
