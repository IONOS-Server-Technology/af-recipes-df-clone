...
# (e.g. "afpre-nats"), so a project-label filter derived from this script's
# own directory ("nats") never matches and silently waits out the full
...
wait_for_healthy() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        ids=$(docker ps -aq --filter "name=(^|-)nats-" 2>/dev/null || true)
        pending=""
        for id in $ids; do
        ...
        if [ -n "$ids" ] && [ -z "$pending" ]; then
            echo "all nats containers are healthy after ${waited}s"
            return 0
        fi
    ...
    echo "ERROR: nats did not become healthy within ${max_wait}s" >&2
    docker ps -a --filter "name=(^|-)nats-" 2>&1 || true
    ids=$(docker ps -aq --filter "name=(^|-)nats-" 2>/dev/null || true)
    for id in $ids; do
...
URL="${1:-http://${SERVERIP:-127.0.0.1}:8222}"
echo "Checking NATS at ${URL}/healthz ..."
